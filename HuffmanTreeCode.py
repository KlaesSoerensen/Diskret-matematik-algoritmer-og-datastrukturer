...
def EncryptData(data, coding):
    encrypted_output = ''.join([coding[ch] for ch in data])
    return encrypted_output


def DecryptData(encoded_data, huffmanTree):
    return HuffmanDecoding(encoded_data, huffmanTree)
...
if __name__ == "__main__":
    ...
    for arg in sys.argv:
        if arg.startswith("table="):
            ...
            to_encrypt = arg.split("=")[1].replace("\"", "")
        elif arg.startswith("decrypt="):
            to_decrypt = arg.split("=")[1].replace("\"", "")
    ...
    if to_encrypt:
        encrypted_output = EncryptData(to_encrypt, the_codes)
        print("Encrypted output:", encrypted_output, " Length:", len(encrypted_output))

    if to_decrypt:
        decrypted_output = DecryptData(to_decrypt, the_tree)
